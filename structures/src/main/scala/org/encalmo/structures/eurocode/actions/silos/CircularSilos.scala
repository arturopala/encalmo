...
object ThinWalledCircularSlenderSilosWithSteepHopperExpressions extends MapContext {
    ...
    this(pnf) = Ff*pv
    this(ptf) = mu_l*Ff*pv
    this(pnf0) = Eval(pnf,x -> he)
    this(ptf0) = Eval(ptf,x -> he)
    ...
    this(pne) = Fe*pv
    this(pte) = mu_l*Fe*pv
    this(pne0) = Eval(pne,x -> he)
    this(pte0) = Eval(pte,x -> he)
    ...
    this(cpi) = -0.6
    this(ze) = h4
    this(Re) = d1*Eval(vmz,z -> ze)/((15*1E-6) unit SI.m2/SI.s)
    this(Aref) = d1*(h4-h3)
    this(cf) = 1.2+((0.18*log(10*((200 unit SI.m)/d1)))/(1+0.4*log(Re/1E5)))
    this(Fw) = cscd*cf*Eval(qpz,z -> ze)*Aref
    ...
    this(NxEd(2)) = (Gck*(h1-lR)+Grk+Gpk)*1.35 + Quk*1.5 + Qsk*U*1.5 + Eval(nezSk, z -> (hc-lR))*U*1.5
