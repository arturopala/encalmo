...
class BetonSprezonyDzwigarTypuI extends CalculationDocument {
    ...
    val np2 = n|"p2" is "Przyjęta liczba cięgien w strefie ściskanej" acc 2; calc(np2) = (np1+1)*(20 unit SI.percent)
    ...
    val h1 = h|1 is "Wysokość półki górnej przekroju" unit SI.cm; calc(h1) = max(np2y*phip+(np2y-1)*dymin+1*cnom+2*phis,7 unit cm)
    ...
    val h5 = h|5 is "Wysokość półki dolnej przekroju" unit SI.cm acc 1; calc(h5) = max(np1y*phip+(np1y-1)*dymin+1*cnom+2*phis,10 unit cm)
    ...
    val Ap1s = A|"p1s" is "Pole sprowadzone cięgien w strefie rozciąganej" unit mm2 acc 1; calc(Ap1s) = (alphae-1)*Ap1
    val Ap2s = A|"p2s" is "Pole sprowadzone cięgien w strefie ściskanej" unit mm2 acc 1; calc(Ap2s) = (alphae-1)*Ap2
    val Acs = A|"cs" is "Pole sprowadzone przekroju" unit SI.cm2 acc 0.1; calc(Acs) = Ac+Ap1+Ap2
    ...
    val Myd = BasicSymbols.M|"yd" is "Obliczeniowy moment zginający" unit SI.kNm acc 0.1; calc(Myd) = qd*(leff^2)/8
    
    val sigma0max = sigma|"0,max" is "Maksymalne naprężenie w sprężanych cięgnach" unit MPa; calc(sigma0max) = min(0.8*fpk,0.9*fp01k)
    val P0max = P|"0,max" is "Początkowa wartość siły sprężającej" unit kN acc 0.1; calc(P0max) = sigma0max*Ap
    val dPir = "ΔP"|"ir" is "Strata spowodowana relaksacją stali sprężającej dla klasy 2 ciegien" unit kN acc 0.1; calc(dPir) = P0max*0.04*0.55
    val Pmo1 = P|"mo1" is "Siła początkowa przed zwolnieniem naciągu" unit kN acc 0.1; calc(Pmo1) = P0max - dPir
    val zcp = z|"cp" is "Mimośród siły sprężającej" unit cm; calc(zcp) = (Ap1*(yd-yp1)-Ap2*(yg-yp2))/Ap
    val dPc = "ΔP"|"c" is "Strata od odkształcalności sprężystej betonu" unit kN acc 0.1; calc(dPc) = alphae*(Ap/Acs)*((1+(zcp)^2)*(Acs/Ics))*Pmo1
    val Pmo2 = P|"mo2" is "Siła sprężająca po zwolnieniu naciągu" unit kN acc 0.1; calc(Pmo2) = Pmo1 - dPc
    val sigpmo = sigma|"pmo" is "Naprężenie w cięgnach po zwolnieniu naciągu" unit MPa; calc(sigpmo) = min(Pmo2/Ap,0.75*fpk,0.85*fp01k)
