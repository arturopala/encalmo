...
object RectangularSiloSymbols extends SymbolConfigurator {
    ...
    val b3 = symbol(BasicSymbols.b|3) unit "m"
    val b4 = symbol(BasicSymbols.b|4) unit "m"
    val h1 = symbol(BasicSymbols.h|1) unit "m"
    val h2 = symbol(BasicSymbols.h|2) unit "m"
    val h3 = symbol(BasicSymbols.h|3) unit "m"
    val be = symbol(BasicSymbols.b|BasicSymbols.e) unit "m"
}

/** RectangularSilos expressions */
object RectangularSlenderSiloWithFlatBottomExpressions extends MapContext {
...
extends Calculation {
	...
	this add RectangularSlenderSiloWithFlatBottomExpressions
	...
	//filling symmetrical load
	def fillingSymmetricalLoad = NumSection(TextToTranslate("_fillingSymmetricalLoad",SiloSymbols.dictionary),
	...
	//filling patch load
	def fillingPatchLoad = NumSection(TextToTranslate("_fillingPatchLoad",SiloSymbols.dictionary),
        Evaluate(ef,Ef,Cpf,ppf,ppfnc,ppfnc1,s,Fpf1)
    )
    //discharge symmetrical load
	def dischargeSymmetricalLoad = NumSection(TextToTranslate("_dischargeSymmetricalLoad",SiloSymbols.dictionary),
        Evaluate(CS,Ch,Cw,phe,phet,pwe,nezSk,nezSkt)
    )
    //discharge patch load
    def dischargePatchLoad = NumSection(TextToTranslate("_dischargePatchLoad",SiloSymbols.dictionary),
        Evaluate(Cpe,ppe,ppenc,ppenc1,Fpe1)
    )
    
    //loads on silo hoppers
    def loadsOnSiloBottom = NumSection(TextToTranslate("_loadsOnSiloBottom",SiloSymbols.dictionary),
        Evaluate(pvft)
    )
